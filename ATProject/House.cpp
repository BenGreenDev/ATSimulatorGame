...
House::House(Graphics& gfx)
{
	if (!IsStaticInitialised())
	{
		...
		AddStaticBind(std::make_unique<Texture>(gfx, Surface::FromFile("Images\\house.png")));
...
void House::init(std::string name, int _level, int _price)
{
	canBeClicked = true;
	buildingName = name;
	level = _level;
...
void House::SetPos(int _xPos, int _yPos, int _zPos) noexcept
{
	xPos = _xPos;
	yPos = _yPos;
	zPos = _zPos;
...
void House::SetScaling(float xScaling, float yScaling, float zScaling)
{
	xScale = xScaling;
	yScale = yScaling;
	zScale = zScaling;
...
void House::SetRotation(float rotation)
{
	yaw = rotation;
...
void House::Update(float dt) noexcept
{
	currentTime += dt;
...
DirectX::XMMATRIX House::GetTransformXM() const noexcept
{
	{
...
bool House::CheckRayIntersection(DirectX::XMVECTOR rayOrigin, DirectX::XMVECTOR rayDirection)
{
