...
Shop::Shop(Graphics& gfx)
{
	if (!IsStaticInitialised())
	{
		AddStaticBind(std::make_unique<VertexBuffer>(gfx, model.vertices));
		AddStaticBind(std::make_unique<Texture>(gfx, Surface::FromFile("Images\\shop.png")));
...
void Shop::init(std::string name, int _level, int _price)
{
	canBeClicked = true;
	buildingName = name;
	level = _level;
...
void Shop::SetPos(int _xPos, int _yPos, int _zPos) noexcept
{
	xPos = _xPos;
	yPos = _yPos;
	zPos = _zPos;
...
void Shop::SetScaling(float xScaling, float yScaling, float zScaling)
{
	xScale = xScaling;
	yScale = yScaling;
	zScale = zScaling;
...
void Shop::SetRotation(float rotation)
{
	yaw = rotation;
...
void Shop::Update(float dt) noexcept
{
	currentTime += dt;
...
DirectX::XMMATRIX Shop::GetTransformXM() const noexcept
{
	{
...
bool Shop::CheckRayIntersection(DirectX::XMVECTOR rayOrigin, DirectX::XMVECTOR rayDirection)
{
