...
Hospital::Hospital(Graphics& gfx)
{
	if (!IsStaticInitialised())
	{
		AddStaticBind(std::make_unique<VertexBuffer>(gfx, model.vertices));
		AddStaticBind(std::make_unique<Texture>(gfx, Surface::FromFile("Images\\hospital.png")));
...
void Hospital::init(std::string name, int _level, int _price)
{
	canBeClicked = true;
	buildingName = name;
	level = _level;
	...
	statsManager->AddJobs(baseNumberOfJobs * level);
}

void Hospital::SetPos(int _xPos, int _yPos, int _zPos) noexcept
{
	xPos = _xPos;
	yPos = _yPos;
	zPos = _zPos;
...
void Hospital::SetScaling(float xScaling, float yScaling, float zScaling)
{
	xScale = xScaling;
	yScale = yScaling;
	zScale = zScaling;
...
void Hospital::SetRotation(float rotation)
{
	yaw = rotation;
...
void Hospital::Update(float dt) noexcept
{
	currentTime += dt;
	...
	if (currentTime > timePerUpdate)
	{
		...
		if (r < 0.5)
		{
			double percentageToCure = ((double)rand() / (double)(RAND_MAX));

			if (percentageToCure < 0.3)
			{
				statsManager->CurePeople(percentageToCure);
			}
		}
...
DirectX::XMMATRIX Hospital::GetTransformXM() const noexcept
{
	{
...
bool Hospital::CheckRayIntersection(DirectX::XMVECTOR rayOrigin, DirectX::XMVECTOR rayDirection)
{
...
void Hospital::RenderUI()
{
	...
	if (ImGui::Begin(buildingName.c_str()))
	{
		...
		{
			statsManager->RemoveJobs(baseNumberOfJobs * level);
...
DirectX::XMFLOAT3 Hospital::GetPos()
{
