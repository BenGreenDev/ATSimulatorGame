...
Mountain::Mountain(Graphics& gfx,
	std::string fileName)
{
...
void Mountain::SetPos(int _xPos, int _yPos, int _zPos) noexcept
{
	xPos = _xPos;
	yPos = _yPos;
	zPos = _zPos;
...
void Mountain::Update(float dt) noexcept
{

}
DirectX::XMMATRIX Mountain::GetTransformXM() const noexcept
{
	namespace dx = DirectX;
...
void Mountain::SetScaling(float xScaling, float yScaling, float zScaling)
{
	xScale = xScaling;
	yScale = yScaling;
	zScale = zScaling;
...
DirectX::XMFLOAT3 Mountain::GetPos()
{
...
bool Mountain::CheckRayIntersection(DirectX::XMVECTOR rayOrigin, DirectX::XMVECTOR rayDirection)
{
