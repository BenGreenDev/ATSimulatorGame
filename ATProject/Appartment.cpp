...
Appartment::Appartment(Graphics& gfx)
{
	if (!IsStaticInitialised())
	{
		AddStaticBind(std::make_unique<VertexBuffer>(gfx, model.vertices));
		AddStaticBind(std::make_unique<Texture>(gfx, Surface::FromFile("Images\\App.png")));
...
void Appartment::init(std::string name, int _level, int _price)
{
	canBeClicked = true;
	buildingName = name;
	level = _level;
	...
	if (level != 1)
	{
		pricePerUpdate = pricePerUpdate * (2 * (level - 1));
	}

	statsManager->AddHomes(baseNumberOfHomes * level);
}

void Appartment::SetPos(int _xPos, int _yPos, int _zPos) noexcept
{
	xPos = _xPos;
	yPos = _yPos;
	zPos = _zPos;
...
void Appartment::SetScaling(float xScaling, float yScaling, float zScaling)
{
	xScale = xScaling;
	yScale = yScaling;
	zScale = zScaling;
...
void Appartment::SetRotation(float rotation)
{
	yaw = rotation;
...
void Appartment::Update(float dt) noexcept
{
	currentTime += dt;

	if (currentTime > timePerUpdate)
	{
		statsManager->UseMoney(pricePerUpdate);
		currentTime = 0.0f;
	}
}

DirectX::XMMATRIX Appartment::GetTransformXM() const noexcept
{
	{
...
bool Appartment::CheckRayIntersection(DirectX::XMVECTOR rayOrigin, DirectX::XMVECTOR rayDirection)
{
...
void Appartment::RenderUI()
{
	...
	if (ImGui::Begin(buildingName.c_str()))
	{
		...
		if (level != 3)
		{
			...
			{
				pricePerUpdate = pricePerUpdate * 2;
				level = level + 1;
				price = newPrice;
		...
		std::string pricePerUp = "Current Price Per Update - " + std::to_string(pricePerUpdate);
		ImGui::Text(pricePerUp.c_str());

		std::string homesProvided = "Current Capacity - " + std::to_string(baseNumberOfHomes * level);
		ImGui::Text(homesProvided.c_str());
		...
		{
			statsManager->RemoveHomes(baseNumberOfHomes * level);
...
DirectX::XMFLOAT3 Appartment::GetPos()
{
